...
\glossupperplural{recovery host}是愿意在复苏背景下提供固定块的钉钉。这些pinners应该表示他们将把这个任务交给发行商，并下载了所有相关的块，并将它们全部固定在本地实例中。
