清单表示字符串到引用的映射(参见\ref{sec:collections})。其主要目的是实现文件集合(网站)的群集，并支持基于url的内容寻址。本节定义与清单相关的数据结构，以及实现清单API的查找和更新算法(参见\ref{sec:manifests-ux}和\ref{spec:api:manifest})。
